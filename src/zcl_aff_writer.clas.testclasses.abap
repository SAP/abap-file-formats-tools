...
*      get_struc_of_enum_values_intf for testing raising cx_static_check,
...
*    data(name_mappings) = value if_aff_type_writer=>ty_name_mappings(
*      ( abap = 'ABAP_ELEMENT' json = 'JSON_ELEMENT' )
*    ).
*
*    cut->if_aff_type_writer~set_name_mappings( name_mappings ).
*
*    cl_abap_unit_assert=>assert_equals( exp = name_mappings act = cut->name_mappings ).
*  endmethod.
*
*  method set_abap_value_mappings.
*    data(abap_value_mappings) = value if_aff_type_writer=>ty_abap_value_mappings(
*      ( abap_element = 'ABAP_ELEMENT' value_mappings = value #( ( abap = '1' json = '2' ) ) )
*    ).
*
*    cut->if_aff_type_writer~set_abap_value_mappings( abap_value_mappings ).
*
*    cl_abap_unit_assert=>assert_equals( exp = abap_value_mappings act = cut->abap_value_mappings ).
*  endmethod.
*
*  method set_formatting_option.
*    cut->if_aff_type_writer~set_formatting_option( if_aff_type_writer=>formatting_option-camel_case ).
*
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>formatting_option-camel_case act = cut->formatting_option ).
*  endmethod.
*
*  method get_output.
...
*
*    data(act_output) = cut->if_aff_type_writer~get_output( ).
*
*    cl_abap_unit_assert=>assert_equals( exp = exp_output act = act_output ).
*  endmethod.
*
*  method camel_case.
*    cut->formatting_option = if_aff_type_writer=>formatting_option-camel_case.
*
...
*    cut->formatting_option = if_aff_type_writer=>formatting_option-no_formatting.
*
*    data(act_name) = cut->map_and_format_name( 'MY_TEst_nAmE' ).
*
...
*    data(value_mappings) = value if_aff_type_writer=>ty_value_mappings( ( abap = 'X' json = 'true' ) ( abap = ' ' json = 'false' ) ).
*    data(abap_value_mapping_1) = value if_aff_type_writer=>ty_abap_value_mapping(
*      abap_element   = 'MY_TEST_ELEMENT_1'
*      target_type    = if_aff_type_writer=>type_info-boolean
*      value_mappings = value_mappings ).
*    data(abap_value_mapping_2) = value if_aff_type_writer=>ty_abap_value_mapping(
*      abap_element   = 'my_test_element_2'
*      target_type    = if_aff_type_writer=>type_info-string ).
*
...
*    data(exp_value_mappings) = value if_aff_type_writer=>ty_value_mappings( ( abap = 'X' json = 'true' ) ( abap = ' ' json = 'false' ) ).
...
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-string act = cut->get_json_type_from_description( get_element_description( char ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-string act = cut->get_json_type_from_description( get_element_description( value string( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-string act = cut->get_json_type_from_description( get_element_description( value char01( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-string act = cut->get_json_type_from_description( get_element_description( value xstring( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-string act = cut->get_json_type_from_description( get_element_description( value if_aff_type_writer=>enum_type_info( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-string act = cut->get_json_type_from_description( get_element_description( value numc04( ) ) ) ).
*  endmethod.
*
*  method get_type_info_boolean.
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-boolean act = cut->get_json_type_from_description( get_element_description( value abap_boolean( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-boolean act = cut->get_json_type_from_description( get_element_description( value abap_bool( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-boolean act = cut->get_json_type_from_description( get_element_description( value flag( ) ) ) ).
*  endmethod.
*
*  method get_type_info_numeric.
*    data packed type p.
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-numeric act = cut->get_json_type_from_description( get_element_description( packed ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-numeric act = cut->get_json_type_from_description( get_element_description( value f( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-numeric act = cut->get_json_type_from_description( get_element_description( value int1( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-numeric act = cut->get_json_type_from_description( get_element_description( value int2( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-numeric act = cut->get_json_type_from_description( get_element_description( value int8( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-numeric act = cut->get_json_type_from_description( get_element_description( value decfloat16( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-numeric act = cut->get_json_type_from_description( get_element_description( value decfloat34( ) ) ) ).
*  endmethod.
*
*  method get_type_info_date_time.
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-date_time act = cut->get_json_type_from_description( get_element_description( value d( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-date_time act = cut->get_json_type_from_description( get_element_description( value t( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-date_time act = cut->get_json_type_from_description( get_element_description( value timestamp( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-date_time act = cut->get_json_type_from_description( get_element_description( value timestampl( ) ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>type_info-date_time act = cut->get_json_type_from_description( get_element_description( value utclong( ) ) ) ).
*  endmethod.
*
*  method stack_stores_operations.
*    data table type standard table of tadir.
*
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>operation-initial act = cut->last_operation( ) ).
*
*    cut->if_aff_type_writer~write_element( element_name = 'write_element' element_description = value #( ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>operation-write_element act = cut->last_operation( ) ).
*
*    cut->if_aff_type_writer~open_node( node_name = 'open_structure' node_description = cl_abap_typedescr=>describe_by_data( value tadir( ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>operation-open_structure act = cut->last_operation( ) ).
*
*    cut->if_aff_type_writer~close_node( node_name = 'close_structure' node_description = cl_abap_typedescr=>describe_by_data( value tadir( ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>operation-close_structure act = cut->last_operation( ) ).
*
*    cut->if_aff_type_writer~open_node( node_name = 'open_table' node_description = cl_abap_typedescr=>describe_by_data( table ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>operation-open_table act = cut->last_operation( ) ).
*
*    cut->if_aff_type_writer~close_node( node_name = 'close_table' node_description = cl_abap_typedescr=>describe_by_data( table ) ).
*    cl_abap_unit_assert=>assert_equals( exp = if_aff_type_writer=>operation-close_table act = cut->last_operation( ) ).
*  endmethod.
*
...
*    data(name_of_source) = `CL_AFF_TEST_TYPES_FOR_WRITER`.
...
*    data abap_doc_exp type cl_aff_abap_doc_parser=>abap_doc.
*    abap_doc_exp = value #( description = `This is an enum` title = `myCategory` enumvalues_link = `cl_aff_test_types_for_writer.data:enum_values` ).
...
*    abap_doc_act = cut->call_reader_and_decode( name_of_source =  name_of_source element_name   = element_name ).
*    abap_doc_exp = value #( title = 'Enum Type' description = 'Enum type' default = '@link cl_aff_test_types_for_writer.data:enum_values.classic_badi' ).
*    cl_abap_unit_assert=>assert_equals( exp = abap_doc_exp act = abap_doc_act ).
*
*  endmethod.
*
...
*    insert value #( name = `MY_STRUCTURE3` absolute_name = `\CLASS=CL_AFF_TEST_TYPES_FOR_WRITER\TYPE=MY_STRUCTURE3` ) into cut->stack_of_structure index 1.
*    insert value #( name = `MY_STRUCTURE3` absolute_name = `\CLASS=CL_AFF_TEST_TYPES_FOR_WRITER\TYPE=MY_STRUCTURE3` ) into exp_table index 1.
*    insert value #( name = `MY_NESTED_STRUCTURE` absolute_name = `\CLASS=CL_AFF_TEST_TYPES_FOR_WRITER\TYPE=MY_NESTED_STRUCTURE` ) into cut->stack_of_structure index 1.
...
*    insert value #( name = `LIST` absolute_name = `\CLASS=CL_AFF_TEST_TYPES_FOR_WRITER\TYPE=LIST` ) into cut->stack_of_structure index 1.
...
*    data(source_exp) = `CL_AFF_TEST_TYPES_FOR_WRITER`.
...
*    data(absolute_name) = `\CLASS=CL_AFF_TEST_TYPES_FOR_WRITER\TYPE=MY_STRUCTURE`.
...
*    data(enumvalues_link) = `cl_aff_test_types_for_writer.data:enum_values`.
...
*    ).
*    data(exp_name_of_source) = `CL_AFF_TEST_TYPES_FOR_WRITER`.
...
*    data(ref) = cl_aff_test_types_for_writer=>enum_values.
...
*  endmethod.
*
*  method get_struc_of_enum_values_intf.
*    data(enumvalues_link) = `if_aff_types_v1.data:co_abap_language_version_src`.
*    cut->get_structure_of_enum_values(
*      exporting
*        link_to_values      = enumvalues_link
*        fullname_of_type    = ``
...
*    ).
*
*    data(exp_name_of_source) = `IF_AFF_TYPES_V1`.
*    data(exp_name_of_constant) = `CO_ABAP_LANGUAGE_VERSION_SRC`.
*    cl_abap_unit_assert=>assert_equals( exp = exp_name_of_constant act = act_name_of_constant ).
*    cl_abap_unit_assert=>assert_equals( exp = exp_name_of_source act = act_name_of_source ).
*
*    data exp_structure type ref to cl_abap_structdescr.
*    data(ref) = if_aff_types_v1=>co_abap_language_version_src.
*    exp_structure = cast cl_abap_structdescr( cl_abap_structdescr=>describe_by_data( ref ) ).
*    cl_abap_unit_assert=>assert_equals( exp = exp_structure->get_components( ) act = act_structure_of_values->get_components( ) ).
*    cl_abap_unit_assert=>assert_equals( exp = exp_structure->kind act = act_structure_of_values->kind ).
*    cl_abap_unit_assert=>assert_equals( exp = exp_structure->type_kind act = act_structure_of_values->type_kind ).
...
*    data(class_name) = `CL_AFF_TYPE_WRITER`.
*    data(is_valid) = cut->is_callback_class_valid( class_name = class_name component_name = 'Component Name' ).
*    data(log) = cut->if_aff_type_writer~get_log( ).
...
*    cl_abap_unit_assert=>assert_true( cut->if_aff_type_writer~validate( source = value #( ) log = value #( ) ) ).
*  endmethod.
*
*  method handle_include.
*    data data type cl_aff_test_types_for_writer=>ty_include_type.
*    cut->if_aff_type_writer~open_include( cast cl_abap_structdescr( cl_abap_structdescr=>describe_by_data( data ) ) ).
*    cl_abap_unit_assert=>assert_equals( exp = 1 act = lines( cut->stack_of_structure ) ).
*    cut->if_aff_type_writer~close_include( ).
